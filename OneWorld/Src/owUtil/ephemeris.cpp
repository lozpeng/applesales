...
static 
double getModifiedJulianDate( struct tm gmt, double lon )
{
	double mjd,timezone;
	int b, d, m, y;
	long c;

	{
		...
		timezone = (floor((lon - 7.5)/15.0)+1)*3600;
...
static void sun_pos2(double mjd,  double& X, double& Y, double& Z)
{
	...
	CalcEquitorialRaDec( mjd, 0.0, lsn, &ra, &dec);
	...
	Z = dis *sin(dec);
	X = dis * cos(dec) * cos(-ha);
	Y = dis * cos(dec) * sin(-ha);
}
...
void Ephemeris::GetSunPos2(double mjd,  double& X, double& Y, double& Z)
{
	sun_pos2(mjd, X, Y, Z);
}

void Ephemeris::GetSunPos( double mjd, 
                        double lat, 
                        double lon,
                        double elev, 
                        double *sun_alt, 
...
double Ephemeris::GetMJD(int year, int month, int day, int hour, int min, int sec,double lon)
{
   struct tm mt;
  ...
  return getModifiedJulianDate(mt,lon);
