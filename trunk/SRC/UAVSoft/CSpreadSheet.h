...
m_bAppend(false), m_bBackup(Backup), m_bTransaction(false)
{
	...
	m_stempString = PathFindExtension(m_sFile);

	m_stempString.MakeLower();
	if (m_stempString == ".xls"||m_stempString == ".xlsx") // File is an Excel spreadsheet
	{
		m_bExcel = true;
...
bool CSpreadSheet::Open()
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		m_Database->OpenEx(m_sDsn, CDatabase::openReadOnly);
...
void CSpreadSheet::GetExcelDriver()
{
	...
	// Search for the driver...+		pszBuf	0x0018d42a 
	const char* excelDriver = "Microsoft Excel Driver (*.xls, *.xlsx, *.xlsm, *.xlsb)";

	do
	{
		if( strstr( pszBuf, excelDriver ) != 0 )
		{
			// Found !
