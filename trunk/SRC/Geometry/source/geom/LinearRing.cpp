/**********************************************************************
 * $Id: LinearRing.cpp 1820 2006-09-06 16:54:23Z mloskot $
 *
...
void LinearRing::ReplacePoint(int index, GEOMETRY::geom::Coordinate &newpoint)
{
    if(index<0 || index>=getNumPoints())
	{
		return;
	}
	size_t ptnum =getNumPoints();

	//如果要替换的是起点或终点，则两个点一起替换
	if(index==0 || index ==(ptnum-1))
	{
         points->setAt(newpoint,0);

		  points->setAt(newpoint,ptnum-1);
	}
	else
	{
