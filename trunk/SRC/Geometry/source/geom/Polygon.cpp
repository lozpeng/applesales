/**********************************************************************
 * $Id: Polygon.cpp 1820 2006-09-06 16:54:23Z mloskot $
 *
...
#include <Geometry/geom/MultiLineString.h> // for getBoundary()
...
void Polygon::SetExteriorRing(GEOMETRY::geom::LinearRing *ring)
{
	if(shell)
	{
		delete shell;
...
Geometry *Polygon::OrganizePolygon()
{
	return this;
}

bool Polygon::IsContainArc() const
{
	//检查外环
	
	if(shell)
	{
       if(shell->IsContainArc())
	   {
		   return true;
	   }
	}
	GEOMETRY::geom::LineString *pString;
	for(size_t i=0;i<holes->size();i++)
	{
		pString =dynamic_cast<GEOMETRY::geom::LineString*>((*holes)[i]);
		if(!pString)
		{
			continue;
		}

		if(pString->IsContainArc())
		{
			return true;
		}
	}
...
*
******************************************************************************************************/
bool Polygon::SnapVertex(Coordinate *pPoint,double tolerance,Coordinate *poutPt,double *searchDis,long *plpartIndex,long *plvertexIndex)
{
	long lnum = GeometryCount();
	if(lnum<=0)
	{
		return false;
	}
    ...
    for(long i=0;i<lnum;i++)
	{
		...
		if(! GetGeometry(i)->SnapVertex(&temppt,tolerance,&temppt1,&distance,&lpart,&lvertexIndex))
		{
			continue;
		}
...
bool Polygon::SnapEdgePoint(Coordinate *pPoint,double tolerance,Coordinate *poutPt,double *searchDis,long *plpartIndex,long *plSegIndex)
{
   long lnum = GeometryCount();
	if(lnum<=0)
	{
    ...
    for(long i=0;i<lnum;i++)
	{
		...
		if(! GetGeometry(i)->SnapEdgePoint(&temppt,tolerance,&temppt1,&distance,&lpart,&lvertexIndex))
		{
			continue;
		}
...
bool Polygon::SnapMidPoint(Coordinate *pPoint,double tolerance,Coordinate *poutPt,double *searchDis,long *plpartIndex,long *plSegIndex)
{
   long lnum = GeometryCount();
	if(lnum<=0)
	{
    ...
    for(long i=0;i<lnum;i++)
	{
		...
		if(! GetGeometry(i)->SnapMidPoint(&temppt,tolerance,&temppt1,&distance,&lpart,&lvertexIndex))
		{
			continue;
		}
...
Geometry* Polygon::GetGeometry(size_t i)
{
   if(i==0)
   {
	   //返回外环
...
void Polygon::Mirror(const Coordinate &p1,const Coordinate &p2)
{
   
    
   for(long i=0;i<GeometryCount();i++)
	{
		GetGeometry(i)->Mirror(p1,p2);
