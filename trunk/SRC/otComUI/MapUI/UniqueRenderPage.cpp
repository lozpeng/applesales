...
BEGIN_MESSAGE_MAP(CUniqueRenderPage, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_COLORBAR, &CUniqueRenderPage::OnCbnSelchangeComboColorbar)
END_MESSAGE_MAP()
...
BOOL CUniqueRenderPage::OnInitDialog()
{
	...
	//获得颜色表
	m_ctlColorRamp.GetColorMap(m_ctlColorRamp.GetCurSel(),m_struColorMap);
...
void CUniqueRenderPage::OnBnClickedUniAddall()
{
	...
	//加入默认符号
    pSymbolArray->Add(m_pDefaultSymbol);

	for ( SCMAP::iterator it = m_SymbolMap.begin() ; it != m_SymbolMap.end() ; it++ )
	{
		pSymbolArray->Add(it->second.pSymbol);
	}
	//列表根据符号数组产生图片
...
void CUniqueRenderPage::OnBnClickedBtnUniOverview()
{
	...
	if(iter!=m_SymbolMap.end())
	{
		...
		sc.pSymbol =symlist.SelectSymbol(sc.pSymbol);
...
Display::ISymbolPtr CUniqueRenderPage::AddUniquerSymbol()
{
	...
	/*m_struColorMap.ucB*/
	BYTE R = (BYTE)(((double) rand() / (double) 255) * 255 + 0);
	COLORREF lColor = RGB(m_struColorMap.ucR[R], m_struColorMap.ucG[R],m_struColorMap.ucB[R]);
...
void CUniqueRenderPage::OnCbnSelchangeComboColorbar()
{
	// TODO: 在此添加控件通知处理程序代码
	m_ctlColorRamp.GetColorMap(m_ctlColorRamp.GetCurSel(),m_struColorMap);
}
