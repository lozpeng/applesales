...
void CUniquerRenderSymbolList::CreateSymbolArrayBitmap(Display::ISymbolArrayPtr m_pSymbolArray)
{
	...
	CDC MemDC;
	CString Str1;
	BOOL b = m_Bitmap.CreateCompatibleBitmap(dc,m_LegendnWidth*m_pSymbolArray->GetSize(),m_LegendnHeight);
	MemDC.CreateCompatibleDC(dc);

	CBitmap *pOldBmp=(CBitmap *)MemDC.SelectObject(&m_Bitmap);
	RECT rc1;
	rc1.top=rc1.left=0;
	rc1.right=m_LegendnWidth*m_pSymbolArray->GetSize();
	...
	HRGN hr = 0;
	long i,lCount =  m_pSymbolArray->GetSize();
	...
	for( i = 0 ; i < lCount ; i++,rc.left+=m_LegendnWidth,rc.right+=m_LegendnWidth )
	{
		...
		SimpleDisplay.DrawLegend( m_pSymbolArray->GetAt(i).get() , &rc, 0 ); 
