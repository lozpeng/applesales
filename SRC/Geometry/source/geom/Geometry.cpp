...
#include <Geometry/algorithm/CentroidArea.h>
#include <Geometry/algorithm/InteriorPointPoint.h>
#include <Geometry/algorithm/InteriorPointLine.h>
...
#include <Geometry/operation/relate/RelateOp.h>
...
#include <Geometry/operation/distance/DistanceOp.h>
...
using namespace GEOMETRY::operation::valid;
using namespace GEOMETRY::operation::relate;
using namespace GEOMETRY::operation::buffer;
using namespace GEOMETRY::operation::overlay;
using namespace GEOMETRY::operation::distance;
	...
	userData(NULL)
{
	...
	//SRID=geom.getSRID();
	//userData=NULL;
    m_bhasZ =geom.m_bhasZ;
    m_bhasM =geom.m_bhasM;
}
	...
	//throw(IllegalArgumentException *)
{
	if(this->isScatter())
	{
		Geometry::AutoPtr pGeometry(scatter());
		if(other->isScatter())
		{
			Geometry::AutoPtr pOtherGeometry(other->scatter());
			return RelateOp::relate(pGeometry.get(), pOtherGeometry.get());
		}
		else
		{
			return RelateOp::relate(pGeometry.get(),other);
		}
	}
	else
	{
		...
		{
			Geometry::AutoPtr pOtherGeometry(other->scatter());
			return RelateOp::relate(this,pOtherGeometry.get());
		}
		else
		{
...
void Geometry::FitEnvelope(const Envelope &destEnv)
{
	Envelope env =*getEnvelopeInternal();
	if(abs(env.getWidth())<0.000001 || abs(env.getHeight())<0.000001)
		return;
	if(abs(destEnv.getWidth())<0.000001 || abs(destEnv.getHeight())<0.000001)
		return;

	double dx =destEnv.getWidth()/env.getWidth();
	double dy =destEnv.getHeight()/env.getHeight();

	Coordinate center,oc;
	destEnv.centre(center);
	env.centre(oc);
	Scale(oc,dx,dy);
	Move(center.x -oc.x,center.y-oc.y);
}
...
* Bug #64 - Not all control paths return a value in GEOMETRY::geom::Geometry::getClassSortIndex
*
