/**********************************************************************
 * $Id: GeometryFactory.cpp 1820 2006-09-06 16:54:23Z mloskot $
 *
	...
	const
{
	...
	for (size_t i=0; i<fromLines.size(); i++)
	{
		...
		if ( ! line ) throw GEOMETRY::util::IllegalArgumentException("createMultiLineString called with a vector containing non-LineStrings");
...
CircularArc* GeometryFactory::createCircularArc(const GEOMETRY::geom::Coordinate &Center, double radius, double startAngle, double centerAngle)  const
{
	return new CircularArc(Center,radius,startAngle,centerAngle);
}

Circle* GeometryFactory::createCircle(const GEOMETRY::geom::Coordinate &Center, double radius) const
{
	return new Circle(Center,radius);
}

Circle* GeometryFactory::createCircle() const
{
	return new Circle();
}

EllipticArc* GeometryFactory::createEllipticArc(const GEOMETRY::geom::Coordinate &Center, double semiMajor, double semiMinor, double rotationAngle, const GEOMETRY::geom::Coordinate &FromPt, const GEOMETRY::geom::Coordinate &ToPt, bool bCounterClock) const
{
	return new EllipticArc(Center,semiMajor,semiMinor,rotationAngle,FromPt,ToPt,bCounterClock);
}

Ellipse* GeometryFactory::createEllipse(const GEOMETRY::geom::Envelope &envelop) const
{
	

	return new Ellipse(envelop);
}

Ellipse* GeometryFactory::createEllipse() const
{
	return new Ellipse();
}


BezierSpline* GeometryFactory::createBezierSpline(const std::vector<Coordinate>& knots) const
{
	return new BezierSpline(knots);
}
