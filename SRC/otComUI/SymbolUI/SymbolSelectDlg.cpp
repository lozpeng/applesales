...
{
	/*CSymbolLibSettingDlg SymbolLibSettingDlg;

	if( SymbolLibSettingDlg.DoModal() != IDOK)
		return;
	CString strName = SymbolLibSettingDlg.GetName();
	CString strPath = SymbolLibSettingDlg.GetPath();
...
void CDlgSymbolSelect::OnAddSymbolMenuItems(UINT nID)
{
	...
	if( slib.AddSymbol( m_pSymbol ) )
		...
		::MessageBox(this->m_hWnd , "符号保存失败：可能重名" , "警告", MB_OK );
}
