...
void CFeatureRenderPage::AddImageButtons()
{
	...
	//dotdensityPage->SetLayer( m_pLayer );
	//m_pages.push_back( dotdensityPage );
	//hIcon = m_icons.ExtractIcon (1);
	//m_wndPane.AddButton (hIcon, "点密度专题", 0, -1, 0);
	//::DestroyIcon (hIcon);

	////饼图图例
	//CPieRenderPage *piePage = new CPieRenderPage();
	//piePage->SetLayer( m_pLayer );
	//m_pages.push_back( piePage );
	//hIcon = m_icons.ExtractIcon (1);
	//m_wndPane.AddButton (hIcon, "饼状图专题", 0, -1, 0);
	//::DestroyIcon (hIcon);

	////柱图图例
	//CPillarRenderPage *pillarPage = new CPillarRenderPage();
	//pillarPage->SetLayer( m_pLayer );
	//m_pages.push_back( pillarPage );
	//hIcon = m_icons.ExtractIcon (1);
	//m_wndPane.AddButton (hIcon, "柱状图专题", 0, -1, 0);
	//::DestroyIcon (hIcon);
}
