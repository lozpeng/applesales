...
CActionEditFeature::CActionEditFeature() : IAction("EditFeature")
{
...
void CActionEditFeature::LButtonDownEvent (UINT nFlags, CPoint point)
{
	...
	if(m_nStatus == On_None )
	{
		...
		otFramework::IAction *pAction = otFramework::IAction::FindAction("SelectByPoint");
		//用点选取要素
...
void CActionEditFeature::LButtonUpEvent (UINT nFlags, CPoint point)
{
	...
	else if(m_nStatus == On_MoveMoreShape)
	{
		...
		if(m_bMoved)
		{
			...
			std::vector<otCarto::ILayer*> vecMapLayers;
			long fid;
...
BOOL CActionEditFeature::GetSelectGeometrys()
{
	...
	std::vector<otCarto::ILayer*> alllayers;
	...
	otCarto::CFeatureLayer* pFeatureLayer =NULL;
	GeodataModel::IFeatureClass *pFeatureClass =NULL;
    ...
    for(size_t i=0;i<alllayers.size();i++)
	{
		...
		pFeatureLayer =dynamic_cast<otCarto::CFeatureLayer*>(alllayers[i]);
		if(!pFeatureLayer)
		{
			continue;
		}
