...
#define DEFAULT_DIVISION_LEN 10
		...
		case LF_ONE:
			{
				...
				DrawText(pDisplay, m_NumTextSymbol, (LPTSTR)cNum, rect, DT_NOCLIP | DT_NOPREFIX | DT_CENTER | DT_TOP);
			}
			break;
		case LF_MAIN_DIVISION:
			{
				for(int i=0; i<m_DivisionNum+1; i++)
				{
					...
					sprintf(cNum, "%3f", (int)number );

					subRect.left = rect.left + i*rect.Width()/(double)m_DivisionNum - 50;
					subRect.right = subRect.left + 100;
		...
		case LF_ALL_DIVISIONS:
			{
				...
				for(int i=1; i<m_SubDivisionNum; i++)
				{
					...
					subRect.left = rect.left + i*rect.Width()/(double)m_DivisionNum/(double)m_SubDivisionNum - 50;
					subRect.right = subRect.left + 100;
					DrawText(pDisplay, m_NumTextSymbol, (LPTSTR)cNum, subRect, DT_NOCLIP | DT_NOPREFIX | DT_CENTER | DT_TOP);
				}
			}
		...
		case LF_DIVISIONS_FIRST_MIDPOINT:
			{
				...
				subRect.left = rect.left + rect.Width()/(double)m_DivisionNum/2.0/* - 50*/;
				subRect.right = subRect.left/* + 100*/;
				DrawText(pDisplay, m_NumTextSymbol, (LPTSTR)cNum, subRect, DT_NOCLIP | DT_NOPREFIX | DT_CENTER | DT_TOP);
			}
			break;
		case LF_DIVISIONS_FIRST_SUBDIVISION:
			{
				...
				subRect.left = rect.left + rect.Width()/(double)m_DivisionNum/(double)m_SubDivisionNum/* - 50*/;
				subRect.right = subRect.left /*+ 100*/;
				DrawText(pDisplay, m_NumTextSymbol, (LPTSTR)cNum, subRect, DT_NOCLIP | DT_NOPREFIX | DT_CENTER | DT_TOP);
			}
			break;
	...
	{
		m_DivisionLen = len;m_contentChanged = TRUE;
	}
	...
	{
		m_DivisionNum = num;m_contentChanged = TRUE;
	}
