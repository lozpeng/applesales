...
inline void Display::CDC::DrawPolygon(DIS_POLYGON &polygon)
{
	...
	::FillRgn( m_hDC , hRgn , (HBRUSH)GetCurrentObject(m_hDC,OBJ_BRUSH) );
	DeleteObject( hRgn );
...
void CDC::SetDC( long hDC, long lWidth , long lHeight )
{
		...
		if(!SetGraphicsMode(m_hDC,GM_ADVANCED))
		{
			//OTFALSERETURN_NOVAL0("Error:the function only can run on win2000 and newer system")
		}
