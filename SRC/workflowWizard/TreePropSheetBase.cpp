...
BEGIN_MESSAGE_MAP(CTreePropSheetBase, SUPER_PROPERTYSHEET)
...
IMPLEMENT_DYNAMIC(CTreePropSheetBase, CMyPropertySheet)
...
:	CMyPropertySheet(pszCaption, pParentWnd, iSelectPage)
{
  Init();
}
...
void CTreePropSheetBase::ActivatePreviousPage()
{
	...
	if (!IsWindow(m_pwndPageTree->GetSafeHwnd()))
	{
		...
		//不再继续跳转
		int	nPrevIndex = (nPageIndex==0)? 0 : nPageIndex-1;
...
void CTreePropSheetBase::ActivateNextPage()
{
  // Window must be defined and a least one property page.
	...
	if (!IsWindow(m_pwndPageTree->GetSafeHwnd()))
	{
		...
		//不继续跳转
		int	nNextIndex = (nPageIndex==GetPageCount()-1)? nPageIndex : nPageIndex+1;
...
BOOL CTreePropSheetBase::OnInitDialog() 
{
	...
	BOOL bResult = CMyPropertySheet::OnInitDialog();
...
LRESULT CTreePropSheetBase::OnIsDialogMessage(WPARAM wParam, LPARAM lParam)
{
  ...
  if (pMsg->message==WM_KEYDOWN && GetKeyState(VK_CONTROL)&0x8000)
  {
	...
	return SUPER_PROPERTYSHEET::DefWindowProc(PSM_ISDIALOGMESSAGE, wParam, lParam);
}
...
BOOL CTreePropSheetBase::PreTranslateMessage(MSG* pMsg) 
{
  if( pMsg->hwnd == GetPageTreeControl()->GetSafeHwnd() )
  {
    ...
    if( !m_bSkipEmptyPages )
      return SUPER_PROPERTYSHEET::PreTranslateMessage(pMsg);
...
void CTreePropSheetBase::UpdateCustomButtons()
{
	//判断节点位置，更新按钮状态
	bool isFirst = false;
	bool isLast = false;
	HTREEITEM	hItem = m_pwndPageTree->GetSelectedItem();
	isLast = (NULL == GetNextItem(hItem)) ? true : false;
	isFirst = (NULL == m_pwndPageTree->GetPrevSiblingItem(hItem)) ? true : false;
	__super::UpdateCustomButtons(isFirst,isLast);
}
int CTreePropSheetBase::DoSkip()
{
	if(0 == DoCustomSkip())
	{
		//跳转到下一页。需完善
...
int CTreePropSheetBase::DoCustomSkip()
{
	return 0;
}
} //namespace TreePropSheet
